...
# When a file that defines Claude Code tooling (agents, skills, settings,
...
# If docs/manuals doesn't exist yet, skip all checks silently
[ ! -d "$MANUALS_DIR" ] && exit 0
...
INSTRUCTIONS
   ...
   - Also add/update a matching row in the appropriate category table (the skill doubles as a command)."
        else
